...
STOPWORDS: Final[frozenset[str]] = frozenset(
    {
        ...
        "in", "into", "is", "isn", "its", "itself", "just", "let", "like", "ll", "may",
        "me", "might", "mine", "more", "most", "much", "must", "my", "myself", "neither",
        "no", "nor", "not", "now", "of", "off", "on", "once", "one", "only", "onto",
        "or", "other", "others", "ought", "our", "ours", "ourselves", "out", "over",
        "own", "per", "re", "same", "shall", "she", "should", "so", "some", "such",
        "than", "that", "the", "their", "theirs", "them", "themselves", "then", "there",
        "these", "they", "this", "those", "though", "through", "thus", "to", "too",
        "under", "until", "up", "upon", "us", "ve", "very", "was", "wasn", "we", "were",
        "weren", "what", "when", "where", "whether", "which", "while", "who", "whom",
        "whose", "why", "will", "with", "within", "without", "won", "would", "wouldn",
...
_DOMAIN_TERMS: Final[dict[str, tuple[str, ...]]] = {
    "embedded": (
        "embedded", "firmware", "bootloader", "mcu", "microcontroller", "soc",
        "bare-metal", "baremetal", "rtos", "freertos", "zephyr", "threadx", "stm32",
        "stm32f4", "stm32h7", "esp32", "esp8266", "nrf52", "nrf53", "atmega", "avr",
        "arduino", "platformio", "stm32cubeide", "cubemx", "hal", "cmsis", "cortex",
        "cortex-m", "cortex-a", "arm", "risc-v", "msp430", "pic32", "teensy",
        "raspberry", "jetson", "interrupt", "interrupts", "isr", "dma", "watchdog",
        "eeprom", "i2c", "spi", "uart", "usart", "can", "canbus", "can-fd", "canopen",
        "modbus", "jtag", "swd", "openocd", "st-link", "segger", "gpio", "adc", "dac",
        "pwm", "peripheral", "peripherals", "datasheet", "register-level", "low-power",
        "yocto", "buildroot", "petalinux", "flashing", "toolchain",
        "embedded systems", "embedded linux", "embedded software", "device drivers",
        "device driver", "driver development", "bare metal", "real-time",
    ),
    "hardware": (
        "pcb", "kicad", "altium", "eagle", "schematic", "soldering", "smd", "reflow",
        "oscilloscope", "multimeter", "analyzer", "fpga", "verilog", "vhdl",
        "systemverilog", "vivado", "quartus", "xilinx", "altera", "asic", "rtl", "hdl",
        "electronics", "circuit", "circuits", "breadboard", "voltage", "analog",
        "sensor", "sensors", "imu", "accelerometer", "gyroscope", "magnetometer",
        "lidar", "encoder", "servo", "stepper", "bldc", "motor", "actuator", "relay",
        "transistor", "mosfet", "amplifier", "antenna", "rf", "ble", "bluetooth",
        "zigbee", "lora", "lorawan", "nfc", "battery", "enclosure", "machining", "cnc",
        "solidworks", "onshape", "autocad", "fusion", "cad", "tolerance", "fabrication",
        "printed circuit", "3d printing", "signal integrity", "power electronics",
    ),
    "robotics": (
        "robot", "robots", "robotic", "robotics", "ros", "ros2", "rclcpp", "rclpy",
        "gazebo", "moveit", "urdf", "tf2", "nav2", "slam", "odometry", "localization",
        "mapping", "kinematics", "manipulator", "gripper", "drone", "uav", "quadcopter",
        "rover", "autonomous", "teleoperation", "waypoint", "ekf", "kalman",
        "rplidar", "velodyne", "realsense", "pcl", "open3d",
        "sensor fusion", "path planning", "motion planning", "point cloud",
        "inverse kinematics", "robot operating",
    ),
    "control": (
        "pid", "lqr", "mpc", "setpoint", "damping", "state-space", "trajectory",
        "closed-loop", "open-loop", "feedforward", "actuation", "servo", "foc",
        "control theory", "motor control", "motion control", "control loop",
        "model predictive",
    ),
    "realtime": (
        "rtos", "freertos", "real-time", "realtime", "latency", "deterministic",
        "jitter", "scheduler", "scheduling", "preemptive", "interrupt", "microsecond",
        "millisecond", "throughput", "hard real-time", "worst-case",
    ),
    "systems": (
        "c", "c++", "rust", "assembly", "asm", "linux", "kernel", "unix", "posix",
        "pthread", "pthreads", "concurrency", "multithreading", "thread", "threads",
        "mutex", "semaphore", "atomic", "memory", "pointer", "pointers", "malloc",
        "allocator", "compiler", "linker", "gcc", "clang", "llvm", "makefile", "cmake",
        "ninja", "bazel", "gdb", "lldb", "valgrind", "profiling", "profiler",
        "optimization", "optimize", "syscall", "elf", "abi", "cross-compile",
        "segfault", "ubuntu", "debian", "bash", "shell", "systemd", "daemon",
        "operating system", "memory management", "low-level",
    ),
    "language": (
        "c", "c++", "c#", "python", "java", "javascript", "typescript", "rust", "go",
        "golang", "kotlin", "swift", "ruby", "php", "perl", "haskell", "scala",
        "elixir", "lua", "luau", "matlab", "dart", "julia", "fortran", "objective-c",
        "sql", "bash", "powershell", "vhdl", "verilog", "gdscript", "solidity",
        "assembly", "programming", "compiler", "syntax", "idiomatic",
    ),
    "ml": (
        "pytorch", "torch", "tensorflow", "keras", "jax", "scikit-learn", "sklearn",
        "xgboost", "lightgbm", "transformer", "transformers", "huggingface", "llm",
        "gpt", "bert", "embedding", "embeddings", "neural", "cnn", "rnn", "lstm",
        "dnn", "training", "trained", "inference", "dataset", "datasets", "overfitting",
        "regularization", "gradient", "backpropagation", "epoch", "epochs",
        "hyperparameter", "fine-tuning", "finetuning", "quantization", "onnx",
        "tensorrt", "cuda", "cudnn", "gpu", "reinforcement", "q-learning", "ppo",
        "rlhf", "classifier", "classification", "regression", "clustering",
    ...
    "cv": (
        "opencv", "cv2", "yolo", "yolov5", "yolov8", "image", "images", "vision",
        "segmentation", "detection", "camera", "calibration", "stereo", "depth",
        "pixel", "pixels", "augmentation", "imagenet", "resnet", "mediapipe",
        "tesseract", "ocr", "pose", "keypoint", "keypoints", "homography", "sift",
        "orb", "canny", "hough", "photogrammetry",
        "computer vision", "object detection", "image processing", "optical flow",
        "feature matching", "semantic segmentation",
    ),
    "data": (
        "pandas", "numpy", "scipy", "matplotlib", "seaborn", "plotly", "jupyter",
        "notebook", "dataframe", "etl", "analytics", "statistics", "statistical",
        "visualization", "bigquery", "spark", "hadoop", "airflow", "dbt", "warehouse",
        "csv", "parquet", "aggregation", "correlation", "outlier", "sampling",
        "data analysis", "data pipeline", "time series", "a/b",
    ),
    "frontend": (
        "react", "reactjs", "react.js", "next.js", "nextjs", "vue", "vuejs", "vue.js",
        "svelte", "sveltekit", "angular", "html", "css", "sass", "scss", "tailwind",
        "tailwindcss", "bootstrap", "jsx", "tsx", "dom", "component", "components",
        "hooks", "redux", "zustand", "vite", "webpack", "rollup", "babel", "eslint",
        "prettier", "responsive", "accessibility", "a11y", "animation", "framer",
        "shadcn", "storybook", "electron", "tanstack", "spa", "ssr", "dashboard",
        "dashboards", "frontend", "front-end", "ui", "viewport",
        "user interface", "design system", "single-page",
    ),
    "backend": (
        "api", "apis", "rest", "restful", "graphql", "grpc", "fastapi", "django",
        "flask", "express", "express.js", "node.js", "nodejs", "spring", "rails",
        "laravel", "microservice", "microservices", "endpoint", "endpoints",
        "middleware", "authentication", "authorization", "oauth", "jwt", "celery",
        "rabbitmq", "kafka", "webhook", "webhooks", "serverless", "backend",
        "back-end", "server", "asgi", "wsgi", "uvicorn", "gunicorn", "pydantic",
        "sqlalchemy", "orm", "throughput", "idempotent",
        "rest api", "message queue", "background job", "rate limiting",
    ),
    "web": (
        "http", "https", "html", "css", "javascript", "browser", "url", "rest",
        "websocket", "websockets", "cors", "dns", "cdn", "seo", "frontend", "backend",
        "fullstack", "full-stack", "web", "website", "webapp", "spa", "session",
        "cookie", "cookies", "ajax", "json",
        "web application", "web app",
    ),
    "mobile": (
        "android", "ios", "swift", "swiftui", "kotlin", "flutter", "dart", "xcode",
        "gradle", "mobile", "smartphone", "apk",
        "react native", "app store", "play store", "mobile app",
    ),
    "cloud": (
        "aws", "ec2", "s3", "lambda", "dynamodb", "cloudfront", "gcp", "azure",
        "cloud", "serverless", "terraform", "cloudformation", "iam", "vpc",
        "cloudwatch", "supabase", "firebase", "firestore", "heroku", "vercel",
        "netlify", "cloudflare", "kubernetes", "k8s", "autoscaling",
        "google cloud", "amazon web",
    ),
    "devops": (
        "docker", "dockerfile", "kubernetes", "k8s", "helm", "ci", "cd", "ci/cd",
        "jenkins", "gitlab", "pipeline", "pipelines", "deployment", "deploy",
        "deployed", "ansible", "terraform", "nginx", "prometheus", "grafana",
        "monitoring", "observability", "logging", "sre", "uptime", "rollout",
        "containerization", "container", "containers", "devops", "infrastructure",
        "provisioning", "orchestration",
        "github actions", "gitlab ci", "continuous integration",
        "continuous deployment", "infrastructure as",
    ),
    "database": (
        "sql", "postgres", "postgresql", "psql", "mysql", "sqlite", "mongodb", "mongo",
        "redis", "dynamodb", "cassandra", "elasticsearch", "pgvector", "index",
        "indexes", "indexing", "query", "queries", "transaction", "transactions",
        "schema", "migration", "migrations", "orm", "normalization", "sharding",
        "replication", "database", "databases", "db", "join", "joins", "rollback",
        "vector database", "full-text",
    ),
    "security": (
        "security", "encryption", "cryptography", "tls", "ssl", "oauth", "jwt",
        "authentication", "authorization", "vulnerability", "vulnerabilities",
        "penetration", "pentest", "owasp", "csrf", "xss", "injection", "firewall",
        "sandbox", "sandboxing", "threat", "exploit", "cve", "hardening",
        "access control", "secure boot", "zero trust",
    ),
    "networking": (
        "tcp", "udp", "ip", "http", "dns", "socket", "sockets", "packet", "packets",
        "wireshark", "tcpdump", "bandwidth", "protocol", "protocols", "ethernet",
        "wifi", "router", "routing", "mqtt", "zigbee", "lora", "ble", "network",
        "networking", "rpc", "grpc", "websocket", "latency", "handshake", "subnet",
        "network protocol", "packet capture",
    ),
    "testing": (
        "test", "tests", "testing", "pytest", "unittest", "jest", "vitest", "mocha",
        "cypress", "playwright", "selenium", "gtest", "googletest", "coverage", "tdd",
        "mock", "mocks", "mocking", "fixture", "fixtures", "assertion", "assertions",
        "qa", "validation", "verification", "benchmark", "benchmarks", "fuzzing",
        "regression", "hil", "debugging", "debugged", "reproducible",
        "unit test", "unit tests", "integration test", "test coverage",
        "hardware-in-the-loop", "test-driven",
    ),
    "gamedev": (
        "unity", "unity3d", "unreal", "godot", "roblox", "luau", "gdscript", "game",
        "games", "gameplay", "gamedev", "sprite", "sprites", "collision", "rigidbody",
        "multiplayer", "netcode", "fps", "engine", "npc", "procedural", "leaderboard",
        "playtesting", "monetization", "rojo",
        "game engine", "game design", "level design", "physics simulation",
        "unreal engine",
    ),
    "graphics": (
        "opengl", "vulkan", "directx", "metal", "webgl", "three.js", "shader",
        "shaders", "glsl", "hlsl", "rendering", "render", "rasterization",
        "raytracing", "mesh", "meshes", "texture", "textures", "shading", "gpu",
        "blender", "maya", "viewport", "vertex", "polygon", "lighting", "animation",
        "ray tracing", "compute shader", "real-time rendering",
    ),
    "design": (
        "figma", "design", "ux", "wireframe", "wireframes", "prototype", "prototyping",
        "typography", "layout", "branding", "accessibility", "usability", "mockup",
        "illustrator", "photoshop", "affinity", "palette", "spacing",
        "user research", "design system", "user experience", "visual design",
    ),
    "leadership": (
        "led", "lead", "leader", "leadership", "mentored", "mentoring", "mentor",
        "managed", "manager", "team", "teams", "captain", "president", "founder",
        "co-founder", "organized", "coordinated", "supervised", "directed", "chaired",
        "recruited", "onboarded", "delegated", "stakeholder", "stakeholders",
        "roadmap", "agile", "scrum", "sprint", "kanban", "prioritization", "hiring",
        "cross-functional", "project management", "team lead",
    ),
    "research": (
        "research", "researched", "paper", "papers", "publication", "published",
        "thesis", "dissertation", "conference", "journal", "ieee", "acm", "arxiv",
        "experiment", "experiments", "experimental", "hypothesis", "methodology",
        "literature", "peer-reviewed", "citation", "lab", "laboratory", "novel",
        "state-of-the-art", "ablation", "reproducibility",
        "research paper", "first author",
    ),
    "writing": (
        "documentation", "docs", "wrote", "writing", "readme", "tutorial", "tutorials",
        "blog", "article", "articles", "presentation", "presented", "report",
        "specification", "spec", "markdown", "latex", "proposal", "whitepaper",
        "authored", "editing",
        "technical writing", "api documentation", "design doc",
    ),
    "education": (
        "university", "college", "bachelor", "bachelors", "master", "masters", "phd",
        "degree", "gpa", "coursework", "semester", "graduated", "graduating", "major",
        "minor", "scholarship", "dean", "honors", "transcript", "student",
        "undergraduate", "graduate", "curriculum", "course", "courses", "seminar",
        "computer science", "electrical engineering", "mechanical engineering",
        "dean's list",
    ),
    "award": (
        "award", "awards", "awarded", "winner", "won", "finalist", "medal", "trophy",
        "champion", "championship", "recipient", "scholarship", "hackathon",
        "competition", "podium", "ranked", "placed", "distinction",
...
def cosine(a: Sequence[float], b: Sequence[float]) -> float:
    ...
    if len(a) != len(b):
        raise ValueError(
            f"cannot compare vectors of different widths: {len(a)} and {len(b)}"
        )
...
class BaseEmbedder(abc.ABC):
    ...
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        if dimension < MIN_EMBEDDING_DIM:
            raise ValueError(
                f"EMBEDDING_DIM must be at least {MIN_EMBEDDING_DIM}, got {dimension}"
            )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"<{type(self).__name__} {self.provider}/{self.model} "
...
class HashingEmbedder(BaseEmbedder):
    ...
    def embed_one(self, text: str) -> list[float]:
        ...
        self._add_family(
            vector, tokens, person=_PERSON_UNIGRAM, weight=UNIGRAM_WEIGHT
        )
        self._add_family(
            vector,
...
class OpenAIEmbedder(BaseEmbedder):
    ...
    async def embed(self, texts: list[str]) -> list[list[float]]:
        ...
        for start in range(0, len(texts), OPENAI_BATCH_SIZE):
            ...
            vectors.extend(
                await self._embed_batch([self._prepare_input(text) for text in window])
            )
    ...
    async def _embed_batch(self, batch: list[str]) -> list[list[float]]:
        ...
        for attempt in range(attempts):
            ...
            if not classified.transient or attempt == attempts - 1:
                ...
                raise EmbeddingError(
                    f"embedding request failed after {attempt + 1} attempt(s): "
                    f"{classified}",
                    provider=self.provider,
                    model=self.model,
                ) from cause
    ...
    def _read_vectors(self, response: Any) -> list[list[float]]:
        ...
        if not isinstance(data, (list, tuple)):
            raise EmbeddingError(
                "embedding response has no 'data' array; the endpoint may not be "
                "OpenAI-compatible",
                provider=self.provider,
                model=self.model,
            )
